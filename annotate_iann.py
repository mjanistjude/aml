...
def addvaf(df, BAM):
    ...
    results = df.apply(
        lambda x: splitreads1.getVAF(BAM, x["#CHROM"], x["POS"], x["SV_type"], x["endchr"], x["endpos"]),
        axis=1
    )

    def _get(res, key):
        # getVAF returns {'error': ...} on failure (e.g. no coverage at
        # either breakpoint, bad BAM path) - fall back to None so those
        # rows come through as NaN instead of crashing the pipeline.
        if not isinstance(res, dict) or "error" in res:
            return None
        return res.get(key)

    df["vaf"] = results.apply(lambda r: _get(r, "VAF"))
    df["Total_Reads"] = results.apply(lambda r: _get(r, "total_coverage"))
    df["split_reads"] = results.apply(lambda r: _get(r, "split_reads_total"))
    df["splitAsplitB"] = results.apply(
        lambda r: f"{_get(r,'split_reads_bp1')},{_get(r,'split_reads_bp2')}" if _get(r, "split_reads_bp1") is not None else None
    )
    df["discAdiscB"] = results.apply(
        lambda r: f"{_get(r,'discordant_reads_bp1')},{_get(r,'discordant_reads_bp2')}" if _get(r, "discordant_reads_bp1") is not None else None
    )

    # New: coverage and VAF at each breakpoint separately.
    df["coverage_bp1"] = results.apply(lambda r: _get(r, "coverage_bp1"))
    df["coverage_bp2"] = results.apply(lambda r: _get(r, "coverage_bp2"))
    df["vaf_bp1"] = results.apply(lambda r: _get(r, "VAF_bp1"))
    df["vaf_bp2"] = results.apply(lambda r: _get(r, "VAF_bp2"))
    df["ratioA"] = results.apply(lambda r: _get(r, "ratioA"))
    df["ratioB"] = results.apply(lambda r: _get(r, "ratioB"))

    # New: surface any per-row failure instead of silently propagating NaNs.
    df["vaf_error"] = results.apply(lambda r: r.get("error") if isinstance(r, dict) and "error" in r else None)
...
def svtype(x):
	if x["#CHROM"] != x["endchr"]:
		return "TRA"
	if x["str1"] == x["str2"]:
		return "INV"          # ++ (0,0) or -- (1,1)
	if x["str1"] == 0 and x["str2"] == 1:
		return "DEL"          # +-
	else:  # x["str1"]==1 and x["str2"]==0
		return "DUP"          # -+
...
def check_fusion(x):
	...
	if (x["Gene_St"]!=x["Gene_End"]) and ("IGR:" not in str(x["site1"]) and "IGR:" not in str(x["site2"])):
		return "Y"
	return "-"
